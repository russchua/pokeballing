...
class _CountingPageState extends State<CountingPage> {
  DateTime specificDate = DateTime(2024, 4, 16, 7, 50, 8);
  Timer? _timer;

  @override
  Widget build(BuildContext context) {
    String formattedDate = DateFormat('MM-dd-yyyy – hh:mm a').format(specificDate);
    // String for seconds
    // String formattedSeconds = DateFormat('ss').format(specificDate);
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Seconds: ${specificDate.second}'),
              SizedBox(width: 10),
            ],
          ),
          SizedBox(width: 10),
          ElevatedButton(
                onPressed: () {
                  ...
                  _timer = Timer.periodic(Duration(seconds: 1), (timer) {
                    setState(() {
                      if (specificDate.second > 0) {
                        specificDate = specificDate.subtract(Duration(seconds: 1));
                      } else {
                        _timer?.cancel();
                      }
                    });
